...
#### Files names 
'purchasings_user_rank.csv'    #with rankings
'purchasings_BIG.csv'    #with rankings
...
df_winter= df[   df.bottler_code==49   ]
df_winter= df_winter[   df_winter.month > 8  ]



df_winter= pd.read_csv(uri.rawcokeon+'/purchasings_CCWJ_WINTER.csv', dtype= dtype0,  nrows=10**8)       
gc.collect()  ;     util.pd_info(df)
...
mj= dict2()
mj['df']=    df_winter.query(' machine_code in ' +str(machine_test) )
mj['user']=   list(mj.df.user_id.unique())
mj['df_alluser']=      df_winter.query(' user_id in ' + str(mj.user) )
...
list_all_machine= pd.read_csv( uri.obppc + '/winter_2016_machinecode.csv'  )
list_all_machine= list_all_machine.values
...
for kk in list_all_machine :
  mj= dict2()
  mj['df']=    df_winter[  df_winter.machine_code == kk ]
  mj['user']=   list(mj.df.user_id.unique())
  mj['df_alluser']=      df_winter.query(' user_id in ' + str(mj.user) )

  mmall[kk]=   {}
  mmall[kk]['product_type_freq']= util.np_list_tofreqdict(mj.df_alluser.product_type.values)
  mmall[kk]['product_name_freq']= util.np_list_tofreqdict(mj.df_alluser.product_name.values)

  # mmall[kk]['product_related']= list( mj.df_alluser.product_name.unique() )

util.save(mmall, uri.obppc + '/obppc_h_machine_all_productfreq_winter2016.pkl')
util.save_test(uri.obppc + '/obppc_h_machine_all_productfreq_winter2016.pkl')
  



#####   Insert into Mmachines dataframe ####################################################
...
Xmat1= np.zeros((16000, 12), dtype= np.float64)
...
df_xmat.to_csv(uri.obppc + '/machine_productype_distribution_winter.csv', index=False )
...
list_all_machine[0]
    ...
    mj_df= df_winter[ df_winter.machine_code == 9266311579] 
...
for kk in list_all_machines :
   df_winter[ df_winter[  [  .query(' machine_code in ' +str(machine_test) )
...
df_winter= df[ df.month > 8 ]

df_pivot1= pd.pivot_table(df_winter, index="machine_code", columns='product_type', 
...
df_winter_ccwj= df_winter[ df_winter.bottler_code == 49 ]   # CCWJ

df_winter_ccwj2= pd_date_splitall(df_winter_ccwj, coldate='purchased_at') 


df_winter_ccwj2.to_csv(uri.rawcokeon+ 'purchasings_winter_ccwj.csv')
