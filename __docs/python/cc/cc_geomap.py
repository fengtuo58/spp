...
DIRSTUDY=      DIRCWD +  '/_project/cokeon/'
DIRSTUDY_SUB=  DIRSTUDY+ '/user/'


uri= dict2({ 'python_cokeon':  r'G:/_project/cokeon/'.replace('/','\\'),
'beacon': 'G:/_data/beacon'.replace('/','\\'),
'data':   r'G:/_data/'.replace('/','\\'),
'raw':   r'G:/_data/raw/'.replace('/','\\'),
...
############
df= pd.read_csv(uri.beacon+'/beaconlogs.csv', nrows=10**8)       


bb_gps= util.np_dict_tostr_key()
...
def peruser_stat(dfi) :
 ...
 d= [amount_sum, nb_machine, nb_drink, nb_total, nb_product_type, nb_location, bottler_code,
     t0, t1, dt,
     ]
 i= ['amount_sum', 'nb_machines', 'nb_product', 'nb_transaction', 'nb_product_type', 'nb_location',
     'bottler_code', 't0', 't1', 'period_sec']
