...
##### 2017 02 06   #########################################################################
df= pd.read_csv(uri.rawcokeon+'/purchasings_20170206.csv', dtype= dtype0, nrows=10**8)  
...
### array([34, 49, 33, 44, 32, 45, 46, 19, 48, 36], dtype=int64)
df_ccwj.bottler_code.unique() 
...
##############  Save File   ################################################################
df.to_csv(uri.rawcokeon+'/purchasings_20170206_BIG.csv', mode='w')  



df.columns

df[['amount', 'price', 'total']].values[2585]
...
map_product_cate= da.csv_todictmap(uri.map + 'map_productname_master.txt') 
...
df_filter1= df[  (df.product_type == 'WATER')  &  (df.bottler_code== 49)  ] 
...
df_filter1_user.to_csv(uri.raw + '/user_csv/user_water_CCWJ.csv')



user_list1=  np.array(df_filter1_user.index[0:2000])
...
### Top machines selling   #################################################################
df_filter1_machine= df_filter1.groupby('machine_code').apply(permachine_stat_all)         # 100k users: 100s,

df_filter1_machine.sort_values(by=['amount_sum'], ascending=0, inplace=True)

df_filter1_machine.to_csv(uri.raw + '/machine_csv/machine_water_CCWJ.csv')
...
#####
#### Machine Profile  ##########################################
df_mi=  df[  df.machine_code.isin( [   9266139125 ]   ) ]

           
           


df_mi_productype= df_mi.groupby('product_type').apply(permachine_stat_all)  
...
for ii in [ 4, 5,  11, 12] :
  dfi=  df1[  (df1.month < ii + 1 )  & (df1.year < 2017 )  ]    

  dfi_month= dfi.groupby(['year']).apply( permonth_stat_all )  
           
  print ii, dfi_month.head()         
...
#### Dec Data Check   #####################################################
df_ccwj_dec=  df_ccwj[  (df_ccwj.month == 12.0 )  ] 
