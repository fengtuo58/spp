...
'''
df=  pd.read_csv(uri.raw+'/vacs_csv/hiroshima/csv/8069002_2016.txt',     skipinitialspace=True,  nrows=10**8)       
df1= pd.read_csv(uri.raw+'/vacs_csv/hiroshima/csv/8069002_2015.txt',     skipinitialspace=True,  nrows=10**8)  
df= pd.concat((df, df1), axis=0)
df.to_csv( uri.raw+'/vacs_csv/hiroshima/csv/8069002_all.txt' , mode="w"  )
...
##### Save on desik
'''
df.to_csv( uri.raw + '/vacs_csv/all_city_big.txt'   )
...
''' df.columns
['-RowNum-', 'マシンコード', 'FSプロモーションコード', 'ボトラー商品コード', 'ボトラー商品コード枝番',
...
####Load from Merge    #####################################################################
df= pd.read_csv(uri.raw+'/vacs_csv/uri/2015_2016/all_city_big_2015_2016.txt',  skipinitialspace=True,  nrows=10**8)


'''
df.to_csv( uri.raw+'/vacs_csv/osaka/8344002_all_big.txt'  , index=False, mode='w' )
...
############################################################################################
df['city']=      "kago"     ;  df["city"]=  df["city"].astype("category")
...
df_product= pd.read_csv( uri.map + '/map_cokeon_productname_master.txt').fillna(0)
df_product.columns
''' [u'js_code', u'product_name', u'brand_en_name', u'brand_name', u'product_type', u'SKU_name', u'canpet', u'bottle_size'],
'''
...
map_productsegment= util.pd_df_todict(df_product, colkey= "js_code" , colval= "segment_size_can" )   
...
#### Add Product segment
df_product.segment_size_can.unique()
...
df[ (df.city=="hiro" )  &  ( df.year==2016)  ].qty_sold.sum()
...
cols= [ u'bashotype', u'bottlesize', u'canpet',
       u'city',  u'date2',
        u'hotcold', u'jscode', u'machine_code',
       u'month',  u'product_size', u'productype',  u'qty_sold',  u'temp_high', u'year', 'product_hot' ]

       
df0= df[ cols ]
df0=  df0[ (df0["productype"] != '') & ( df0.bottlesize > -1) & (df0.temp_high > -10) ]
...
df1= df0[df0.month.isin([9, 10, 11, 12 ])]
...
df_pvt_temp[ "-2"]= df_pvt_temp[ "-1"].apply( lambda x : ratio(x, type1="n_day")  )
df_pvt_temp[ "WATER-PET-555-2"]= df_pvt_temp[ "-1"].apply( lambda x : ratio(x, type1="n_machine")  )


df_pvt_temp2= copy.deepcopy(df_pvt_temp)
for x in df_pvt_temp2.columns :
   if x not in [  "-1", "-2", "WATER-PET-555-2", "nan-555-2", "nan-525-1", "nan-280-1",  "nan-0", "nan-280", "nan-500",  'year', 'city', 'temp_high'] :
      df_pvt_temp2[x]= df_pvt_temp2[x] / df_pvt_temp[ "-2"]
...
def getkey(x) :
   return  str(x.name[0]) + str(x.name[1])  + str(x.name[2])  + str(x.name[3])
...
df_pvt_temp['-1']=  df_pvt_temp.apply( lambda x :  getkey(x), axis=1  )
df_pvt_temp[ "-2"]= df_pvt_temp[ "-1"].apply( lambda x : ratio(x, type1="n_day")  )
df_pvt_temp[ "WATER-555-2"]= df_pvt_temp[ "-1"].apply( lambda x : ratio(x, type1="n_machine")  )
...
df_pvt_temp2= copy.deepcopy(df_pvt_temp)
for x in df_pvt_temp2.columns :
   if x not in ["nan-0", 'WATER-PET 1000',  'WATER-PET-300',   '-', "nan-280", "nan-500", "-1", "-2", "-9", '', 'BLACK TEA-PET-470', 'BLACK TEA-PET-425'] :
     df_pvt_temp2[x]= df_pvt_temp2[x] / df_pvt_temp['WATER-PET 1000' ]



df_pvt_temp3= pd.pivot_table(df0, index= ['productsegment'], columns=['city'],  
                          values='qty_sold', aggfunc='sum' )
...
############################################################################################
#####By City - Product Segment Saitou location Type  ##############################################################
#### Adjust by Date frequency for each temperature  ----------------------------------------
df1["bashotype"]= df1["bashotype"].astype('str')   #Remove Category issues


split_col= ["year", "city", 'productsegment', "temp_high" ] 
df_pvt_count_machine= df1.groupby(split_col ).agg({ "date2":         lambda x: x.nunique(),
                                                    "machine_code":  lambda x: x.nunique()      })
...
#####  All Year avg + Product Type +Bottle Size         
df_pvt_temp= pd.pivot_table(df1, index= split_col, columns=['bashotype'],  
                          values='qty_sold', aggfunc='sum' )
df_pvt_temp.fillna(0.0, inplace=True)
df_pvt_temp=  df_pvt_temp.loc[(df_pvt_temp.sum(axis=1) != 0), (df_pvt_temp.sum(axis=0) != 0)]
# df_pvt_temp.reset_index()                                  
