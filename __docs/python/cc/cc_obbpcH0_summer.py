...
df= pd.read_csv(uri.rawcokeon+'/purchasings_BIG.csv', dtype= dtype0,  nrows=10**8)       
...
map_mach_basho= da.csv_todictmap(uri.map + 'map_fsmaster.txt') 


map_mach_basho['areacode']
...
################ Mapping: Product_name        ##############################################
map_product_cate= da.csv_todictmap(uri.map + 'map_productname_category.txt') 
map_product_cate['brand_en_name']['ジョージア エスプレッソブレンド ギフト']
map_product_cate['brand_en_name']['ﾘｱﾙｺﾞｰﾙﾄﾞ ﾜｰｸｽ']
map_product_cate['brand_en_name']['一 茶花']

map_product_cate2=  map_product_cate['brand_en_name']


map_product_cate['brand_en_name']['ｺｶ･ｺｰﾗ']
...
#### Summer  Filtering       ###############################################################
df= pd.read_csv(uri.rawcokeon+'/purchasings_BIG.csv', dtype= dtype0,  nrows=10**8) 
df_summer=  df[ df.month < 10 ]




df_ccwj_summer=  df_ccwj[   df_ccwj.month < 10  ]



#### Sub-Select Machine Selection ##########################################################
df_ccwj_summer_mch= df_ccwj_summer.groupby('machine_code').apply( permachine_stat_all )
df_ccwj_summer_mch.sort_values(by=[ 'amount_sum' ], ascending=0, inplace= True)



#  df_ccwj_summer_mch.to_csv(uri.raw + '/machine_csv/machine_summer_2016.csv' )


list_all_machine=  pd.read_csv( uri.raw + '/obppc_csv/summer_2016_machinecode.csv'  )


import utilgeo as ug

ug.gis_cart_distmatrix()
...
#### Data  Saving       ####################################################################
df.to_csv(uri.rawcokeon+'/purchasings_BIG.csv', index= False, mode='w', index= False)


df_ccwj_summer.to_csv(uri.rawcokeon+'/purchasings_CCWJ_SUMMER.csv', index= False, mode='w')



df_ccwj_summer= pd.read_csv(uri.rawcokeon+'/purchasings_CCWJ_SUMMER.csv', dtype= dtype0,  nrows=10**8)       
gc.collect()  ;     util.pd_info(df)
...
####  Study  for one machine  ##############################################################
machine_test= [  9300047350   ]

machine_test= [  9266143921, 9266101229, 9300047350   ]

machine_test= [  9266143921   ]

machine_test= [  9300030300   ]





#### Details for Machine j #################################################################
mj= dict2()
mj['df']=     df_summer.query(' machine_code in ' +str(machine_test) )
mj['user']=   list(mj.df.user_id.unique())
mj['df_alluser']=       df_summer.query(' user_id in ' + str(mj.user) )
mj['product_related']=  list( mj.df_alluser.product_name.unique() )

mj['product_name_freq']= util.np_list_tofreqdict(mj.df_alluser.product_name.values,
                                                     wweight= mj.df_alluser.amount.values)

mj['product_code_freq']= util.np_list_tofreqdict(mj.df_alluser.js_code.values,
                                                     wweight= mj.df_alluser.amount.values)


mj['product_code_freq_i']= util.np_list_tofreqdict(mj.df.js_code.values,
                                                     wweight= mj.df.amount.values)





#  Print list of all products related to user k ---> machine_j  ############################
tot= len(mj.df_alluser.product_name.values)
for k,x in mj['product_related_freq'].items() :
   print k, ',', x, ',',  np.round(x*1.0 / tot,4),  ',' ,map_product_cate['product_type'][k]
...
############################################################################################
##### Machines in VM  sUMMER  ##############################################################
list_all_machine=  pd.read_csv( uri.raw + '/obppc_csv/summer_2016_machinecode_5800_FINAL.csv'  )
list_all_machine=  list(list_all_machine.values.T[0])
list_all_machine=  [ int(x) for x in list_all_machine ]
...
############################################################################################
########## H0 by Product JS Code ###########################################################
mmall={}
for ii, kk in enumerate(list_all_machine) :
  # if ii > 5 : break 
  # mj= dict2()
  mj_df=          df_summer[  df_summer.machine_code == kk ]
  mj_user=        list(mj_df.user_id.unique())
  mj_df_alluser=  df_summer.query(' user_id in ' + str(mj_user) )

  mmall[kk]=   {}
  #mmall[kk]['product_type_freq']= util.np_list_tofreqdict(mj_df_alluser.product_type.values,
  #                                                     wweight= mj_df_alluser.amount.values)
  
  mmall[kk]['product_code_freq']= util.np_list_tofreqdict(mj_df_alluser.js_code.values,
                                                       wweight= mj_df_alluser.amount.values)
...
# df_all= dfa
ii= -1
for imachine, vv in mmall.items() :  
   ii+=1
   a=   vv['product_code_freq']
   dfa= pd.DataFrame.from_dict(a, orient= 'index')
   dfa.sort_values(by=0, ascending= False, inplace=True)         
   
   dfa.columns= ['freq']
   dfa['product_code']=  dfa.index
   dfa['product_rank']=  1+ np.arange(0, len(dfa))   

   ntot= dfa['freq'].sum() *1.0
   dfa['product_proba']= dfa['freq'] /  ntot

   dfa['machine_id']=  imachine   

   if ii == 0 :
      df_all= dfa
   else :
      df_all= pd.concat((df_all, dfa), axis=0, ignore_index= True)


df_all.to_csv(uri.raw+'/obppc_csv/h0_byproductcode.csv', index=False, mode='w')   
...
### JS code mapping
dfjs= df_summer[['js_code', 'product_name']]

dfjs.drop_duplicates(['js_code'], inplace=True)
############################################################################################



mch_list= df_summer[  df_summer.month < 8  ] .machine_code.unique()



df2= df[  (df.month < 9) & (  df.is_ccwj==1 )   ]


               
mch1= df2.groupby('machine_code').sum()
mch1.sort_values(by='amount', ascending=False, inplace= True)


mch1.to_csv( uri.raw+'/obppc_csv/machine_all_summer.csv' , mode='w'    )


by='SMALL_AREA_NAME').sum()
...
for ii, kk in enumerate(list_all_machine) :
  ...
  mmall[kk]['product_name_freq']= util.np_list_tofreqdict(mj_df_alluser.product_name.values,
                                                       wweight= mj_df_alluser.amount.values)
...
# df_all= dfa 
for imachine, vv in mmall.items() :  
   a=   vv['product_name_freq']
   dfa= pd.DataFrame.from_dict(a, orient= 'index')
   dfa.sort_values(by=0, ascending= False, inplace=True)         
   
   dfa.columns= ['freq']
   dfa['product_name']=  dfa.index
   dfa['product_rank']=  1+ np.arange(0, len(dfa))   
...
df_all.to_csv(uri.raw+'/obppc_csv/h0_byproductname.csv', index=False)   


#### Mapping to OBBPC Product
a=   mmall[kk]['product_name_freq']

aa = np.arra [ [k,v] for k, v in a.items() ]




# mmall[kk]['product_related']= list( mj.df_alluser.product_name.unique() )

  
util.save(mmall, uri.raw + '/obppc_csv/obppc_h_machine_all_productfreq_summer2016.pkl')
util.save_test(uri.raw   + '/obppc_csv/obppc_h_machine_all_productfreq_summer2016.pkl')
############################################################################################
...
#####   Insert into Machines dataframe #####################################################
dd=        mmall[kk]['product_type_freq']   
vv_freq1=  np.array([ v for v in dd.values() ])  
vv_freq2=  vv_freq1 / np.sum(vv_freq1+0.0)
vv_keys1 = [ k for k in dd ]
...
map_type_coli= {
 'BLACK TEA': 0,   'COCA':      1,   'COFFEE':  2,
 'ENERGY':    3,   'JUICE':     4,   'LACTIC':  5,
 'NST':       6,   'OTHER':     7,   'SSD' : 8, 
 'SODA':      8,   'SPORTS':    9,   'WATER':   10,
 '0': 11, '3':11, np.nan: 11
 }


##### Check 
df_error=    df[  df.product_type== '0' ].product_name.unique()
 
 
 
 

####### Calc All_Machine, Product type distribution ########################################
Xmat1= np.zeros((6000, 13), dtype= np.float64)
col1= [ 'machine_code', 'BLACK_TEA', 'COCA', 'COFFEE', 'ENERGY', 'JUICE',  'LACTIC',  'NST',  'OTHER', 'SODA', 'SPORTS', 'WATER', 'X' ]

list_all_machine= set( list_all_machine )
list_all_machine= list( list_all_machine )
  
jj=-1
for kk in list_all_machine :
  jj+=1 
  dd=        mmall[kk]['product_type_freq']   
  vv_freq1=  np.array([ v for v in dd.values() ])  
  vv_freq2=  vv_freq1 / np.sum(vv_freq1+0.0)
  ...
  Xmat1[jj,0] = kk
  for tt, coli in enumerate(dd)  :
     Xmat1[jj, 1+map_type_coli[coli] ]= vv_freq2[tt]   #Put the Freq in Table


df_xmat= pd.DataFrame(Xmat1, columns=col1)
df_xmat.to_csv(uri.raw + '/obppc_csv/machine_productype_distribution_summer.csv', index=False, mode='w')
...
####### Calc All_Machine, Product type Frequency ########################################
Xmat2= np.zeros((6000, 12), dtype= np.float64)
col1= [ 'machine_code', 'BLACK_TEA', 'COCA', 'COFFEE', 'ENERGY', 'JUICE',  'LACTIC',  'NST',  'OTHER', 'SODA', 'SPORTS', 'WATER' ]
...
for kk in list_all_machine :
  ...
  Xmat2[jj,0] = kk
  for tt, coli in enumerate(dd)  :
     Xmat2[jj, 1+map_type_coli[coli] ]= vv_freq2[tt]   #Put the Freq in Table


df_xmat2= pd.DataFrame(Xmat2, columns=col1)
df_xmat2.to_csv(uri.raw + '/obppc_csv/machine_productype_freq_summer.csv', index=False )
...
#########   MANUAL Check        ############################################################
  ...
  mj= dict2()
  mj['df']=    df_summer[  df_summer.machine_code == kk ]
  mj['user']=   list(mj.df.user_id.unique())
  mj['df_alluser']=      df_summer.query(' user_id in ' + str(mj.user) )


  mj['product_type_freq']= util.np_list_tofreqdict(mj.df_alluser.product_type.values,
                                                       wweight= mj.df_alluser.amount.values)
...
tot= mj.df_alluser.amount.sum()
for k,x in mj['product_type_freq'].items() :
    print k, ',', x, ',',  np.round(x*1.0 / tot,4),  ',' 
...
BLACK_TEA	COCA	COFFEE	  ENERGY  JUICE LACTIC NST OTHER	SODA	SPORTS	  WATER
...
map_product_cate= da.csv_todictmap(uri.map + 'map_productname_category.txt') 
map_product_cate['cokeon_type']['い・ろ・は・す ﾊｽｶｯﾌﾟ']
map_product_cate2= map_product_cate['cokeon_type'] 



  
df2= df[['product_name', 'product_type']].drop_duplicates()
  

  
  
print    mj['product_related']  
...
list_all_machine[0]
    ...
    mj_df= df_summer[ df_summer.machine_code == 9266311579] 
...
##### Machines in VM  Winter     ###########################################################
###### CCWJ   
df_summer= df[ df.month < 10 ]
df_summer.to_csv(uri.rawcokeon+ 'purchasings_BIG_CCWJ_SUMMER.csv')




#### Machines codes in VM Summer :  April - End of September 
m_code_summer= df_summer.machine_code.unique()


m_code_all= np.array( df.machine_code.unique())




##### Reconcile cokeon sales vs cokeon summer   ############################################
####  2018 Summer better results



###Load csv   ##############################################################################
df_mcode_summer = pd.read_csv()
...
list_all_machines= df.machine_code.unique()
for kk in list_all_machines :
   df_summer[ df_summer[  [  .query(' machine_code in ' +str(machine_test) )
...
df['day']=     df[coldate].apply(day).astype('int8') 
df['month']=   df[coldate].apply(month).astype('int8') 
df['year']=    df[coldate].apply(year).astype('int16') 
df['hour']=    df[coldate].apply(hour).astype('int8') 
df['season']=  df[coldate].apply(season).astype('int8') 
df['daytime']= df[coldate].apply(daytime).astype('int8') 
gc.collect()  
df['weekday']= df[coldate].apply( weekday ).astype('int8') 
gc.collect()  
print  df.columns
...
#### Add product type: Lambda is super Fast with dict in memory
df['product_type']= df['product_name'].apply(lambda x : map_product_cate2[x])
print  df.columns
...
####Big Pivot:  product category, Winter  ##################################################
df_summer= df[ df.month > 8 ]

df_pivot1= pd.pivot_table(df_summer, index="machine_code", columns='product_type', 
                          values='amount', aggfunc='sum' )
gc.collect()
df_pivot1.fillna(0, inplace=True)

df_pivot1.to_csv(uri.out1+'pivot_cokeon_winter_machine_category_amount.csv')
...
#Print of all products into a file #########################################################
util.pd_extract_col_uniquevalue_tocsv(df, colname='product_name', 
                                  csvfile= uri.map + 'purchasings_product_name.csv') 
...
#Filter by period  -------------------------------------------------------------------------
df['month']= df['purchased_at'].apply(month)   
df_summer= df[ df.month > 8 ]

df_summer_ccwj= df_summer[ df_summer.bottler_code == 49 ]   # CCWJ

df_summer_ccwj2= pd_date_splitall(df_summer_ccwj, coldate='purchased_at') 


df_summer_ccwj2.to_csv(uri.rawcokeon+ 'purchasings_winter_ccwj.csv')
...
mj['product_related_freq']= util.np_list_tofreqdict(mj.df_alluser.product_name.values)
...
tot= len(mj.df_alluser.product_name.values)
for k,x in mj['product_related_freq'].items() :
   print k, ',', x, ',',  np.round(x*1.0 / tot,4),  ',' ,map_product_cate2[k]
...
#  dd1= csv_todictmap(uri.map + 'map_user_rank_group.csv', colname_asindex='user_id') 
user_group= pd.read_csv(uri.map + 'map_user_rank_group.csv',  nrows=10**3)    
...
df= pd.read_csv(rawcokeon+ 'purchasings_user.csv', dtype= dtype0, usecols= usecols, nrows=10**8)       
...
### Save 
df.to_csv(rawcokeon+ 'purchasings_user2.csv', index=False)
gc.collect()  
############################################################################################
...
################                   #########################################################
print("fe")
a= util.os_file_listall("G:/_data/vm_excel", pattern="*.*", dirlevel=10)
...
#### 1) Read CSV FULL: partial columns,   ##################################################
df= pd.read_csv(rawvm+ '4334with20a41a34out20.txt', nrows=10**2)       
gc.collect()       
util.pd_info(df)     
...
df0= pd.read_csv(rawvm+ '4334with20a41a34out20_.txt', dtype= dtype0, usecols= usecols, nrows=2*10**8)           
gc.collect()     
...
df_pivot1= pd.pivot_table(df, index="FS納品場所CD", columns='obppc_type', 
                          values='total', aggfunc='sum' )
gc.collect()
df_pivot1.fillna(0, inplace=True)
df_pivot1.to_csv(out1+'pivot_vm_machine_obppctype_201604_201612.csv')
util.save(df_pivot1, 'pivot_vm_machine_obppctype_201604_201612')



###### Need to find Un-balance machines  ###################################################
...
df= pd.read_csv(rawvm+ '4334with20a41a34out20_.txt', dtype= dtype0, usecols= usecols, nrows=10**8)           
gc.collect()       
util.pd_info(df)     
df.head()
...
df= pd.read_csv(python_cokeon+ '44_utf8.txt', dtype= dtype0, usecols= usecols, nrows=10**8)           
gc.collect()       
util.pd_info(df)     
df.head()
...
pd.DataFrame(a, columns=['col']).to_csv(python_cokeon+ 'all_product_name.txt')
...
ii=0
with open(python_cokeon + 'user_info.csv', 'w') as f:
