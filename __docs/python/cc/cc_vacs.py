...
"売価単価": "int16",            "売上本数": "int32", 
"売上換算ケース数": "float32",   "実売金額":  "float32" 
}

############################################################################################
#### Txt Files into 1 Big CSV
df=  pd.read_csv(uri.raw+'/vacs_csv/osaka/csv/7315006_2014.txt',     skipinitialspace=True,  nrows=10**8)       

dfi= pd.read_csv(uri.raw+'/vacs_csv/osaka/csv/7315006_2017.txt',     skipinitialspace=True,  nrows=10**8)  
dfi.columns

df= pd.concat((df, dfi), axis=0)
'''
/vacs_csv/osaka/7315006_all_big.txt


df.to_csv(uri.raw+'/vacs_csv/osaka/7315006_all_big.txt', index=False, mode='w' )
'''
...
''' df.columns
[u'-RowNum-', u'マシンコード', u'FSプロモーションコード', u'ボトラー商品コード', u'ボトラー商品コード枝番',
       u'コラムNO', u'統合商品コード', u'ボトラーコード', u'FS納品場所コード', u'エリアコード',
       u'光通信前回収集年月日時分', u'精算年月日', u'年月日', u'精算年月', u'売切時間カウント数',
       u'フルカップ補充ケース換算数', u'実機内在庫数', u'補充本数', u'配送換算ケース数', u'バラ数',
       u'FS標準小売バラ単価_税込', u'伝票計上単価', u'売価単価', u'売掛原価単価', u'プロモーション値引',
       u'プロモーション金額', u'プロモーション金額_費用', u'ベースリベート洗替金額', u'ベースリベート金額', u'実売金額',
       u'売上原価', u'CVM売上原価', u'プロモーション売上数', u'プロモーション売上数_費用', u'光通信前回総売上金額',
       u'セット売上金額', u'標準売上金額', u'売上換算ケース数', u'売上本数', u'ホットコールド区分', u'飲料フラグ',
       u'設定機内在庫数'],
...
df= pd.read_csv(uri.raw+'/vacs_csv/osaka/7315006_all_big.txt',     skipinitialspace=True,  nrows=10**8)
df.columns
...
cols0= [ "マシンコード",  "FS納品場所コード",  "光通信前回収集年月日時分", "精算年月日",   "統合商品コード",  "コラムNO", 
         "実機内在庫数", "売切時間カウント数", "売価単価", "売上本数", "売上換算ケース数",  "実売金額" ]      
...
df.columns= [ "machine_code", "basho_code", "datetime", "datesettl", "jscode", "column_nb", 
              "qty_real", "tukire_count", "price", "qty_sold", "qty_sold_sku", "sales_real_yen" ]
...
dtype0= {
"machine_code": "int64",     "basho_code": "int64",      
"datetime" : "int64",        "datesettl" : "int32", 
"jscode": "int32",           "column_nb": "int8", 
"qty_real":    "int32",      "tukire_count": "int32", 
"price": "int16",            "qty_sold": "int32", 
"qty_sold_sku": "float32",   "sales_real_yen":  "float32" 
}
############ Load Big Files ################################################################
...
df1[ df1.year==2015].qty_sold.sum()





util.pd_info_memsize(df_sku)
...
def crate(row) :
   ...
   if per < 1.0 :
     ss= min( 10.0, row['qty_sold'] / (0.00001 + per )  ) 
   else :                
     ss= min( 14.0, row['qty_sold'] / (0.00001 + per  )  )
   return ss
...
df_uri.to_csv(uri.raw + '/vacs_csv/osaka/7315006_urikire.txt', mode='w', index=False)
...
df["year"]= df["date2"].apply( lambda x : add_year(x) )
...
############################################################################################
int(arrow.get( -1 ).format('YYYYMMDD'))
df[ [ 'datetime','date2'] ]


for x in df_temp.cityjap.unique():
   print x 
...
def add_temp(row, city="大阪") :
  key= city + "-" +  str( row)
  try :       return map_temp[key]
  except :    return -100

  
df["temp_high"]= df['date2'].apply( lambda row :  add_temp(row, "福岡")  )
...
'''
df[["datetime","temp_high"]]

13232513  20170217122800         12
13232514  20170224143600          9
13232515  20170224143600          9
'''
...
########### Product Category / Product Size  ###############################################
df_fsmaster= pd.read_csv( uri.map + '/map_fsmaster_unerry.txt')
df_fsmaster.columns
...
'''
df_fsmaster.fillna('0', inplace=True)
...
for x in  df_fsmaster["basho_type"].unique() :
   print x
...
'''
df= pd.read_csv(uri.raw+   '/vacs_csv/fukuoka/8344002_2015.txt'  ,  nrows=10**8)
...
####     df.to_csv(uri.raw+'/vacs_csv/hiroshima/8069002_all_big.txt', index=False, mode='w' )
...
df['product_size']= df.apply( lambda x : str(x["productype"]) + "-" + str(x["bottlesize"]), axis=1)
df['product_size']= df['product_size'].astype("category")

gc.collect()
   

df= df.astype( {"bashotype": "str" ,  "temp_high": "int16", "year" : "int8"  }  )





### df.to_csv(uri.raw+'/vacs_csv/all_city_big.txt', index=False, mode='w' )  ###############

 df= pd.read_csv( uri.raw+'/vacs_csv/all_city_big.txt'  )
...
############################################################################################
### Temperature profile by monthe period
# 04 5 6 7 8  9    10 -->  03   (10, 11 ,12, 01, 02,03 )
df1=  df[ (df["productype"] != '') & ( df.bottlesize > -1) & (df.temp_high > -10) ]
df1= df1[ (df1.date2 < 20170101 ) & (df1.date2 > 20141231 ) ]
df1= df1[ (df1.year > 2000 )  ]

# df1['productype'].fillna('COFFEE', inplace=True)


df1= df1[ df1.city =="osk"]


############################################################################################
df1= df1[df1.month.isin([4, 5, 6 ,7 , 8, 9 ])]


df1= df1[df1.month.isin([10, 11, 12 , 1, 2, 3 ])]
...
df_pvt_temp['nan-0']=   df_pvt_temp.apply( lambda x : str(x.name[0]) + str(x.name[1])  + str(x.name[2]) + str(x.name[3]) , axis=1  )
df_pvt_temp["nan-280"]= df_pvt_temp["nan-0"].apply( lambda x : ratio(x, type1="n_day")  )
df_pvt_temp["nan-500"]= df_pvt_temp["nan-0"].apply( lambda x : ratio(x, type1="n_machine")  )
df_pvt_temp["nan-525"]= 0
