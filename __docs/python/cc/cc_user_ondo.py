...
dtype0= {'amount': 'int16',    'area_code': 'int32',         'bottler_code': 'int16',
 'bottler_name': 'category',   'id': 'int32', 'js_code': 'int32',           'location_code': 'int32',
 'machine_code': 'int64',      'price': 'int16', 'product_name': 'category',   'promotion_id': 'float16',
 'purchased_at': 'object',     'temperature': 'int16',  'user_id': 'int32', 'month': 'int8',  'day': 'int8' ,
  'year': 'int16'   
}
...
#  df.to_csv(uri.rawcokeon+'/purchasings_20170206_BIG.csv', mode='w', index=False)  
...
df['inout'] =   df[ "basho_id" ].apply(addcol_inout)


def addcol_bashotype(x) :
   try :      return  map_basho_type['Location_business_condition'][x]
   except :   return -1
df['bashotype']=   df[ "basho_id" ].apply(addcol_bashotype)
...
######
# df['date']= df.apply( lambda x:  x['year']*10000 + x['month']*100 + x['day'] , axis=1) 




####Load FS VM Address   ###################################################################
df_addr0= pd.read_csv(   uri.map + r'map_vm_address3.txt' )

'''
df_addr0['city']=   df_addr0['adress'].apply( lambda x :  str_to_unicode(x)[0:2]  )
df_addr0['iscokeon'].fillna( 0.0, inplace=True )
df_addr0['city']= df_addr0['city'].apply(    str_convert_beforeprint )
df_addr0.to_csv( uri.map + r'map_vm_address3.txt' ,  index=False, mode='w'   )
'''

####  Add City to mapping    ###############################################################
# map_areacode_city= {}

city_list= ( "大阪",  "京都",  "岡山", "福岡" , "和歌山", "広島", 
             "鳥取", "鹿児島", "東京", "横浜", "熊本",   "神戸"    )


for city in city_list :
  df_addr=      df_addr0[  df_addr0.adress.str.contains(city) ]
  df_addr_list= list( df_addr["area_code"].values)
  for x in df_addr_list :
    try :     a= map_areacode_city[x]
    except :  map_areacode_city[x]= city


def addcity(x) :
   try :     return map_areacode_city[x]
   except :  return ''

df['city']= df['basho_id'].apply( addcity   )
len(df[ df.city != '' ])
...
#### Filter by city name   #################################################################
df_addr= df_addr0[  df_addr0.adress.str.contains(r"大阪市") ]

                   
                  
##### Add columns to Dataframe  
util.pd_col_addfrom_dfmap(df1, df_addr, "area_code", "city", 
                          df_colused="basho_id", df_colnew="citynew2", exceptval='', inplace=True) 



#####  FS Master in Dataframe     ##########################################################
df_map1= pd.read_csv( uri.map + '/map_fsmaster_unerry.txt')
df_map1.fillna('0', inplace=True)
dtype0= {'machine_code': 'int64',    'basho_id': 'int64' }
df_map1= df_map1.astype(dtype0)



##### FS Master 2                 ##########################################################
df_map2= pd.read_csv( uri.map + '/map_fsmaster.txt')
df_map2.fillna('0', inplace=True)
df_map2= df_map2.astype( {'machine_code': 'int64',    'basho_id': 'int64' })

df_map2.columns
'''
[u'machine_code', u'SalesHeadquartersCode', u'SalesDivision', u'area',
       u'No', u'Commercialbusinesscode', u'Commercialflowbranchname',
       u'Commercialroute', u'localarea_code', u'Logisticsofficename',
       u'Logisticsroute', u'Customernumber', u'Customername', u'basho_id',
       u'basho_name', u'basho_type', u'Strategychannel', u'Modelcode',
       u'Modelname', u'Modelyear', u'VMcolumn', u'PETcolumn', u'Notcovered',
       u'flag', u'Annualprospects', u'Salesrank', u'Unnamed: 26']
'''
...
#### Size SKU :
util.pd_col_addfrom_dfmap(df_map1, df_map2, "machine_code", "VMcolumn", 
                          df_colused="machine_code", df_colnew="VMcolumn", exceptval=0, inplace=True) 


len(  df_map1["VMcolumn"].unique()   )
...
############################################################################################
df_map1[ df_map1.machine_code== df_map2['machine_code'].values[0] ]


map_dict= util.pd_df_todict(df_map2, colkey= "machine_code" , colval= "VMcolumn" )  


map_dict[ df_map2['machine_code'].values[0]  ]

  


#### Filter by city name   #################################################################
df_addr= df_addr0[  df_addr0.adress.str.contains(r"鹿児島") ]
...
df["canpet"]=      df["js_code"].apply(add_canpet      )
df["bottlesize"]=  df["js_code"].apply(add_bottlesize  )
len( df[ df["canpet"] == '' ] )
len( df[ df["bottlesize"] == 0 ] )
...
def add_temp(row) :
   key= row["city"]  + "-" + str( row["year"]*10000 + row["month"]*100  + row["day"] )
   try :
      return map_temp[key]
   except :
      return -100
...
df["temp_high"]=  df[[ "year", "month", "day", "city"  ]].apply(add_temp, axis=1 )
len( df[ df["temp_high"] !=  -1 ] )
gc.collect()

  


###  Filter Reduce Amount of records   #####################################################  
#df1=  df[  df.basho_id.isin(df_addr.area_code)   ] 
# df['city'].fillna('' , inplace=True)  
df1=  df[ (df.city != '') & (df.canpet != '') & (df.bottlesize != 0) ]     
# df1=  df1[  ((df1.month < 11) & (df1.month > 4 ))  ]
df1= df1[ df1.temp_high > -1  ]  #Temp positive
df1= df1[ df1.mindate < 20160701  ]    # Start Date of VM



###### Pivot  Temperature  vs Product Type
df_pivot_temp= pd.pivot_table(df1, index="temp_high", columns='product_type', 
                              values='amount', aggfunc='sum' )

df_pivot_temp['total']=  df_pivot_temp.sum( axis=1 )
df_pivot_temp.fillna(0.0, inplace=True)




#### Group By Unique Days 
df_pivot_temp_countday= df1.groupby("temp_high").agg({"amount": np.sum, "id": count,
                        "date": lambda x: x.nunique(),  "machine_code":  lambda x: x.nunique()    
                       })

len(df1.date.unique())


df_pivot_temp_countday= df1.groupby("temp_high").date.nunique()


len( df1["temp_high"].unique() )

util.np_list_tofreqdict(  )

 np_list_tofreqdict(dfi.location_code.values)
 
 
 
df_pivot_temp_countday2= df1.groupby(['temp_high', 'date']).agg({'date': 'count'})
 
 
 df1[ ['temp_high', 'date']].drop_duplicates()
 
 
 
 

###### Pivot  Temperature  vs Can PET
df_pivot_temp= pd.pivot_table(df1, index="temp_high", columns='canpet', 
                              values='amount', aggfunc='sum' )
...
###### Pivot  Temperature  vs Bottle Size
df_pivot_temp= pd.pivot_table(df1, index="temp_high", columns='bottlesize', 
                              values='amount', aggfunc='sum' )
...
###### Pivot  Temperature  vs Product_type, Bottle Size
df_pivot_temp= pd.pivot_table(df1, index="temp_high", columns=[ 'product_type', 'bottlesize'], 
                              values='amount', aggfunc='sum' )
...
df1['bashotype'].fillna('', inplace=True)





df1= df1[ df1.bashotype.str.contains(r"f.娯楽施設")  ]

          


#### Print                
df1[ [u'machine_code', 'js_code',  u'product_name',    u'amount', u'purchased_at', 
      u'is_ccwj',     u'month',    u'day',    'weekday',  u'product_type', 
      u'basho_id',     'bashotype', 'inout',  'city', 'mindate', 'temp_high' ] ].to_csv( uri.raw + '/temperature/cokeon_citylist_ccw_summer.csv',  index=False,  mode='w')     
      ...
      ###### Find Minimun Start date forr VM   ###################################################
def add_df_mindate(dfi):
   dfi.sort_values('purchased_at', inplace=True, ascending=1)
   mindate= 10000 * dfi.year.values[0] + 100 * dfi.month.values[0] + dfi.day.values[0]

   machine_code= dfi.machine_code.values[0]

   d= [ machine_code,    mindate ]
   i= [ 'machine_code', 'mindate' ]
   return pd.Series(data=d, index=i)
   
df_mindate= df.groupby('machine_code').apply(add_df_mindate )
map_df_mindate= util.pd_df_todict(df_mindate, colkey='machine_code')

df['mindate']= df['machine_code'].apply( lambda x : map_df_mindate[x])
...
'''
Index([u'id', u'area_code', u'bottler_name', u'bottler_code', u'machine_code',
       u'location_code', u'js_code', u'product_name', u'price', u'temperature',
       u'promotion_id', u'amount', u'purchased_at', u'user_id', u'no',
       u'is_ccwj', u'year', u'month', u'day', u'weekday', u'daytime',
       u'season', u'total', u'product_type', u'product_brand', u'iscafelait',
       u'basho_id', u'inout', u'bashotype', u'city', u'mindate'],
      dtype='object')
